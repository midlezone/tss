...
class Albums extends ActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('cat_id, album_id, album_name, album_description, album_type, photocount, alias, site_id, user_id,meta_keywords , meta_description, avatar, created_time, modified_time, avatar_path, avatar_name, avatar_id, ishot, order', 'safe'),
        );
    }
    ...
    public static function getAlbumshot($options = array()) {
        ...
        $albums = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('albums'))
                ->where("site_id=$siteid AND ishot=" . self::ALBUMS_HOT)
                ->order('created_time DESC')
                ->limit($options['limit'])
                ->queryAll();
