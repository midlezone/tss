...
class Posts extends ActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('category_id, status, site_id, user_id, created_time, modified_time, modified_by, publicdate', 'numerical', 'integerOnly' => true),
            array('title', 'length', 'max' => 100),
            array('sortdesc, alias', 'length', 'max' => 500),
            array('image_path', 'length', 'max' => 255),
            array('alias', 'isAlias'),
            array('id, category_id, title, sortdesc, description, alias, status, meta_keywords, meta_description, site_id, user_id, image_path, image_name, created_time, modified_time, modified_by, publicdate, avatar,meta_title', 'safe'),
        );
    }
    ...
    public static function countPostInCate($cat_id = 0) {
        ...
        $count = Yii::app()->db->createCommand()->select('count(*)')->from(ClaTable::getTable('post'))
                ->where("site_id=$siteid AND category_id=" . $cat_id)
                ->queryScalar();
        return $count;
    }
