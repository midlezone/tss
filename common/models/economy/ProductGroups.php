    ...
    static function getProductInGroup($group_id, $options = array()) {
        ...
        if ($group_id) {
            $products = Yii::app()->db->createCommand()->select()
					...
					->join(ClaTable::getTable('product_categories') . ' k', 'p.product_category_id=k.cat_id')
            ...
            foreach ($products as $product) {
                ...
                $results[$product['id']]['price_text'] = Product::getPriceText($product);
                $results[$product['id']]['price_market_text'] = Product::getPriceText($product, 'price_market');
            }
            return $results;
        }
        return array();
