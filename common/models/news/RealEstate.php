...
class RealEstate extends ActiveRecord {
    ...
    public function tableName() {
        return ClaTable::getTable('real_estate');
    }
    ...
    public static function getAllRealestate($options = array()) {
        ...
        $data = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('real_estate_news'))
                ->where($condition, $params)
                ->order('created_time DESC')
    ...
    public static function countAllRealestate() {
	 ...
	 /**
     * Tìm tin bất động sản
     * @param type $options
     */
    static function SearchRealestate($options = array()) {
        $results = array();
        if (!isset($options))
            return $results;
    
		$arrArea = explode("|",$options['area']);
		$arrPrice = explode("|",$options['price']);
		...
		if ($options['category'] != '') {
			if ($options['province_id'] != '' ) {
				$condition = "site_id=:site_id AND `status`=" . ActiveRecord::STATUS_ACTIVED ." AND (name LIKE :name) AND cat_id=:cat_id AND province_id=:province_id AND district_id=:district_id ";
				$params = array(
					':site_id' => $siteid,
					':name' => '%' . $options['name'] . '%',				
				...
				$condition = "site_id=:site_id AND `status`=" . ActiveRecord::STATUS_ACTIVED ." AND (name LIKE :name) AND cat_id=:cat_id";
				$params = array(
					':site_id' => $siteid,
					':name' => '%' . $options['name'] . '%',				
					':cat_id' => $options['category'],
			...
			if ($options['province_id'] != '') {
				$condition = "site_id=:site_id AND `status`=" . ActiveRecord::STATUS_ACTIVED ." AND (name LIKE :name) AND province_id=:province_id AND district_id=:district_id ";
				$params = array(
						':site_id' => $siteid,
						':name' => '%' . $options['name'] . '%',			
						':province_id' => $options['province_id'],			
		...
		// if ($options['area'] != '' && $options['category'] == '' && $options['price'] == '') {
			// $condition = "site_id=:site_id AND `status`=" . ActiveRecord::STATUS_ACTIVED ." AND (name LIKE :name)";
			// $params = array(
				// ':site_id' => $siteid,
				// ':name' => '%' . $options['name'] . '%',				
			...
			// );
		// } 
		// if ($options['price'] != '' && $options['category'] == '' && $options['area'] == '') {
			// $condition = "site_id=:site_id AND `status`=" . ActiveRecord::STATUS_ACTIVED ." AND (name LIKE :name) ";
			// $params = array(
				// ':site_id' => $siteid,
				// ':name' => '%' . $options['name'] . '%',				
		...
		$realestate = Yii::app()->db->createCommand()->select('*')
				->from(ClaTable::getTable('real_estate_news'))
                ->Where($condition, $params)
                ->order('created_time DESC')
                ->limit($options['limit'], $offset)
                ->queryAll();
		...
		if ($options['price'] != '') {
			 $realestate = Yii::app()->db->createCommand()->select('*')
				->from(ClaTable::getTable('real_estate_news'))
                ->Where($condition, $params)
                ->andWhere('price >= '.$arrPrice['0'].' ')
                ->andWhere('price <= '.$arrPrice['1'].' ')
                ->order('created_time DESC')
                ->limit($options['limit'], $offset)
                ->queryAll();
		} 
		if ($options['area'] != '') {
			 $realestate = Yii::app()->db->createCommand()->select('*')
				->from(ClaTable::getTable('real_estate_news'))
                ->Where($condition, $params)
                ->andWhere('area >= '.$arrArea['0'].' ')
                ->andWhere('area <= '.$arrArea['1'].' ')
                ->order('created_time DESC')
                ->limit($options['limit'], $offset)
                ->queryAll();
		} 
        if ($options['area'] != '' && $options['price'] != '') {
			 $realestate = Yii::app()->db->createCommand()->select('*')
				->from(ClaTable::getTable('real_estate_news'))
                ->Where($condition, $params)
				->andWhere('price >= '.$arrPrice['0'].' ')
        ...
        $results = array();
        foreach ($realestate as $p) {
			$results[$p['id']] = $p;
			$results[$p['id']]['link'] = Yii::app()->createUrl('/news/realestateNews/detail', array('id' => $p['id'], 'alias' => $p['alias']));
			$results[$p['id']]['full_address'] = $p['province_name'] . ' - ' . $p['district_name'];
    ...
    static function searchTotalCount($options = array()) {
        ...
        $realestate = Yii::app()->db->createCommand()->select('count(*)')
				->from(ClaTable::getTable('real_estate_news'))
                ->where($condition, $params);
