<div class="product-filter-box">   
    <?php
    if ($priceFilter) {
        Yii::app()->controller->renderPartial('//modules/productFilterInCat/priceFilter', array('key' => $key, 'attribute' => $attribute, 'formUrl' => $formUrl));
    }
    ?>
    <?php
    if ($manufacturersOptions) {
        Yii::app()->controller->renderPartial('//modules/productFilterInCat/manufacturersFilter', array('key' => $key, 'manufacturers_options' => $manufacturersOptions));
    }
    ?>
    <?php
    if ($count = count($attributes)) {
        foreach ($attributes as $key => $attribute) {
            Yii::app()->controller->renderPartial('//modules/productFilterInCat/partial/' . $attribute['att']['frontend_input'], array('key' => $key, 'attribute' => $attribute));
        }
    }
    ?>
</div>
