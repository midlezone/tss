                    ...
                    <div class="form-group no-margin-left no-margin-right">
                        <div class="input-group email1">
                            ...
                            <span class="input-group-addon" style="font-size: 14px;">
                                <?php
                                echo CHtml::ajaxSubmitButton(Yii::t('common', 'signup'), Yii::app()->createUrl('site/site/receivenewsletter'), array('type' => 'POST',
                                    'data' => "js:$('#newsletter-form').serialize()",
                                    'success' => 'function(data){var datacb = JSON.parse(data);if(datacb.code == 200){alert("Bạn đã đăng kí thành công");$("button.close" ).trigger( "click");}}',
                                        ), array('class' => 'btn btn-sm', 'id' => 'newslettersubmit', 'style' => 'padding:0; background:#73af43; color:#fff; border-radius: 0px;  display: block;'));
                                ?>
                            </span> 
                        </div>
                        <div>
