...
(function($) {
	$.fn.panorama = function(options) {
		this.each(function(){ 
			...
			$(pano_element).after('<div class="pano_loading_masque" style="position: absolute; top: 0; left: 0; width: '+pano_width+'px; height: '+pano_height+'px;  filter:alpha(opacity=50);"></div>');
					 ...
					 $(pano_element).parent().find("img.vue"+i).bind('load', function(){
						...
						$(pano_element).parent().css("cursor", "pointer"); 
							...
							$(pano_element).bind('mousemove', function(e){
      								if (pano_mouse_down) {
									pano_mouse_delta_x = parseInt((pano_mouse_position_x - e.clientX)/36);
									pano_mouse_delta_y = parseInt((pano_mouse_position_y - e.clientY)/36);
